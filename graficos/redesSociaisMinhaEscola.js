...
async function redesSociaisFavoritasMinhaEscola() {
    const dadosLocaisString = localStorage.getItem('respostaRedesSociais6')
    if (dadosLocaisString) {
        const dadosLocais = JSON.parse(dadosLocaisString)
        processarDados(dadosLocais)
    } else {
        const url = 'https://raw.githubusercontent.com/VendedorDeGalos/api/refs/heads/main/redes.json'
        const res = await fetch(url)
        const dados = await res.json()
        localStorage.setItem('respostaRedesSociais6', JSON.stringify(dados))
